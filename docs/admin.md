...
`GET /api/v1/dashboard` returns public tunnel data only. It intentionally excludes admin-only configuration such as access policies, tokens, rate limits, inspector settings, full client IPs, audit data, and database/runtime internals. Country data comes only from the client report sent at registration and refreshed while connected. The server does not read proxy country headers or resolve countries locally from IP addresses. The configured data directory defaults to `$HOME/.http-tunnel`.
