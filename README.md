...
默认情况下，server 配置、SQLite 数据库和本地数据文件会保存在 `$HOME/.http-tunnel`，client 配置和运行态文件也使用同一个目录。常见配置项会保存到 `$HOME/.http-tunnel/server.toml`，也可通过命令行参数或环境变量覆盖。公开 dashboard 的地图使用国家级热力图，不暴露精确坐标；Cloudflare proxy 场景只读取可信代理传入的 `CF-Connecting-IP` / `X-Forwarded-For` 来识别 client IP。国家信息只采用 client 注册时主动上报并每小时刷新的 country 数据，server 不读取代理国家 header，也不根据 IP 本地解析国家。更完整的字段说明见 [Admin 文档](docs/admin.md) 和 [Security 文档](docs/security.md)。
